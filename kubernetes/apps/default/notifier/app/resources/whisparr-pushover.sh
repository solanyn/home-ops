...
: "${APPRISE_WHISPARR_PUSHOVER_URL:?Pushover URL required}"

echo "[DEBUG] Whisparr Payload: ${PAYLOAD}"
...
function notify() {
    ...
    *)
    ...
    apprise -vv --title "${PUSHOVER_TITLE}" --body "${PUSHOVER_MESSAGE}" --input-format html \
        "${APPRISE_WHISPARR_PUSHOVER_URL}?url=${PUSHOVER_URL}&url_title=${PUSHOVER_URL_TITLE}&priority=${PUSHOVER_PRIORITY}&format=html"
}
